...
User_String = str(input("Please enter what you'd like to search:  ")) 

def find_urls(string):
	if ("https://www." in string):
		url_list = re.findall(r"(https://www\.*\.[\S]{2,})", string)
	elif("http://www." in string ):
		url_list = re.findall(r"(http://www\.*\.[\S]{2,})", string)
	else:
		url_list =re.findall(r"(http\S*\.[0-9A-z,/]{2,})", string)

	return url_list
print(find_urls(User_String))
